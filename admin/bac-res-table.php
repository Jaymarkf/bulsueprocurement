		...
		<table cellpadding="0" cellspacing="0" border="0" id="example1" class="display" cellspacing="0" width="100%">
			...
			<tbody>
				<?php					
					...
					$query4 = mysqli_query($conn,"SELECT * FROM tbl_quotation WHERE Year = $Year3 ORDER BY Company ASC");
				...
				<tr>
					...
					<!--	<a data-placement="top" title="View Purchase Request Detail" id="view" href="app_pr_approved-view.php<?php echo '?pr='.$PR; ?>" class="btn btn-inverse"><i class="icon-eye-open icon-large"></i><br/><span class="badge badge-primary"></a>
						...
						<a data-placement="top" title="Print BAC Resolution" id="print" href="bac-res-preview.php<?php echo '?qID='.$quotationID; ?>" class="btn btn-success"><i class="icon-print icon-large"></i><br/><span class="badge badge-primary"></a>
							<script type="text/javascript">
							$(document).ready(function(){
								$('#print').tooltip('show');
								$('#print').tooltip('hide');
