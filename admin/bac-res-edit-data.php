...
$query = "SELECT * FROM tbl_quotation WHERE Year = $Year AND extPrice <> 0 AND itemDescription = '$item' GROUP BY itemDescription" or die(mysql_error());
					...
					<div class="control-group">
					  <div class="controls">
						...
						<!-- <select name="item_request" placeholder = "Name of articles being requestitioned" class="span12" required>
							...
							<?php							
								//$query = mysql_query("SELECT * FROM tbl_quotation WHERE Year = '$Year' AND extPrice <> 0 AND itemDescription = '$item' GROUP BY itemDescription")or die(mysql_error());
								//while($row = mysql_fetch_array($query)){
								//$id = $row['bacresID'];
