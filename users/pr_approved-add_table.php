		...
		<table cellpadding="0" cellspacing="0" border="0" id="example1" class="display" cellspacing="0" width="100%">
			...
			<tfooter>
				<tr>
					...
					<?php
						//$row = mysqli_query($conn,"SELECT SUM(TotalAmount) as Totalx FROM tbl_pr_items WHERE Year=$Year AND user_id='$session_id'  AND Status = 'Pending'") ;
						$row = mysqli_query($conn,"SELECT SUM(TotalCost) as Totalx FROM tbl_pr_items WHERE Year=$Year AND PRno = '$PR'") ;				
