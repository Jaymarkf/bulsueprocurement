		...
		<table cellpadding="0" cellspacing="0" border="1" id="" class="display" cellspacing="0" width="100%">
			...
			<tbody>
				<?php					
					...
					$query4 = mysql_query("SELECT * FROM tbl_ppmp WHERE Year = $Year3 AND user_id='$user_id3'  AND Status = 'Requested' AND BO_PPMP_Status <> 'Approved' AND SourceOfFund = '$viewby'")or die(mysql_error());
			...
			<tfooter>
				<tr>
					...
					<?php
						$row5 = mysql_query("SELECT SUM(TotalAmount) as Totalx FROM tbl_ppmp WHERE Year = $Year3 AND EndUserUnit='$id'  AND Status = 'Requested' AND BO_PPMP_Status <> 'Approved' AND SourceOfFund = '$viewby'") or die(mysql_error());
